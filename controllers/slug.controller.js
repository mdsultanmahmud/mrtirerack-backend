...
module.exports.getDataForSlugURL = async (req, res, next) => {
  ...
  try {
    ...
    if (!result) {
      const cursor = await db.collection("LearnHowBlog").findOne({
        mainHeading: `${slugUrl}`,
      });
      if (!cursor) {
        ...
        res
          .status(200)
          .send({ success: false, message: "Data is found", data: cursor });
      }
    } else {
      if (!result) {
        res.status(404).send({ success: false, message: "Data is not found" });
        ...
        res
          .status(200)
          .send({ success: false, message: "Data is found", data: result });
      }
    }
  } catch (error) {
    next(error);
