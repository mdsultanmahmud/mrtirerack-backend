...
module.exports.AddBestTire = async (req, res, next) => {
  const db = getDb();
  const tire = req.body;
  tire.createdAt = new Date();
  tire.updatedAt = new Date();
  try {
    ...
    res.status(200).send({
      success: true,
      message: `tire added successfully with id ${cursor.insertedId}`,
    });
  } catch (error) {
    next(error);
  }
...
module.exports.DeleteATire = async (req, res, next) => {
  ...
  try {
    ...
    if (result.deletedCount > 0) {
      res
        .status(200)
        .send({ status: true, message: "Deleted successfully", data: result });
    } else {
      res.status(404).send({
        status: false,
        message: "Tire not found or delete operation failed",
      });
    }
  } catch (error) {
    next(error);
