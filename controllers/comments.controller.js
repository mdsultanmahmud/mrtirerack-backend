  ...
  try {
    ...
    if (comments) {
      return res.status(200).send({
        success: true,
        message: "We get all comments!",
        data: comments,
      });
    } else {
      return res
        .status(404)
        .send({ success: true, message: "There are no comments!" });
    }
  } catch (error) {
    next(error);
  }
...
module.exports.addAComment = async (req, res, next) => {
  ...
  try {
    ...
    if (result.insertedId) {
      ...
      return res
        .status(404)
        .send({ success: true, message: "Comment is not added" });
    }
  } catch (error) {
    next(error);
  }
...
module.exports.getCommentWithBlogId = async (req, res, next) => {
  ...
  try {
    ...
    if (comments) {
      return res.status(200).send({
        success: true,
        message: "We get all comments for this blog!",
        data: comments,
      });
    } else {
      return res.status(404).send({
        success: true,
        message: "There are no comments for this blog!",
      });
    }
  } catch (error) {
    next(error);
...
module.exports.deleteAComment = async (req, res, next) => {
  ...
  try {
    ...
    if (comments.deletedCount > 0) {
      ...
      return res
        .status(404)
        .send({ success: true, message: "Comment is not Deleted!" });
    }
  } catch (error) {
    next(error);
  }
...
module.exports.editComment = async (req, res, next) => {
  ...
  try {
    ...
    if (result.matchedCount === 1) {
      ...
      return res
        .status(404)
        .send({ success: true, message: "Comment is not updated!" });
    }
  } catch (error) {
    next(error);
  }
